...
describe("photon", () => {
  ...
  it("propose", async () => {
    ...
    while (tx == null) {
      ...
      expect((counter += 1)).lte(
        30,
        "Propose transaction has not been found in time",
      );
    }
    ...
    expect(
      (event.data.protocolAddress as Buffer).compare(Buffer.alloc(20, 1)),
    ).eq(0, "Unexpected protocolAddress");
    expect(
      (event.data.functionSelector as Buffer).compare(
        utf8.encode("ask1234mkl;1mklasdfasm;lkasdmf__"),
      ),
    ).eq(0, "Unexpected params");
    expect(
      (event.data.params as Buffer).compare(utf8.encode("an arbitrary data")),
    ).eq(0, "Unexpected data");
    expect((event.data.nonce as anchor.BN).toNumber()).eq(
      1,
      "Unexpected nonce",
    );
  });
