use std::str::FromStr;
use crate::ZeroSumConfig;
use crate::{error::OmnichainError, OperatorInfo, Role, TournamentParams, ZS_PROTOCOL_ID, ZS_ROOT};
...
pub struct PhotonMsg<'info> {
    ...
    #[account(
        signer,
        seeds = [photon::photon::ROOT, b"CALL_AUTHORITY", ZS_PROTOCOL_ID],
        bump,
    ...
    #[account(
        seeds = [ZS_ROOT, b"OPERATOR", executor.key().as_ref()],
        bump,
    ...
    pub zs_config: Box<Account<'info, ZeroSumConfig>>,

    /// ZS Token mint authority
