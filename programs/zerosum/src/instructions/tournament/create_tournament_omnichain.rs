use crate::{
    error::TournamentError, state::TournamentParams, ClaimableUserInfo, OperatorInfo, Role, Tournament, TournamentStatus, ZeroSumConfig, MAX_TEAMS_SIZE, MIN_ORGANIZER_ROYALTY, MIN_TEAMS_SIZE, MIN_TEAM_PLAYERS_CAPACITY, ZS_ROOT
};
...
pub struct CreateTournamentOmnichain<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", sponsor.key().as_ref()], bump, constraint = (operator_info.role == Role::OWNER || operator_info.role == Role::MESSENGER) && operator_info.approved @ TournamentError::OperatorNotApprovedOrInvalidRole)]
    ...
    #[account(
        init_if_needed,
        payer = sponsor,
        seeds = [ZS_ROOT, b"USER", sponsor.key().as_ref()],
        bump,
