use crate::{FeeMeta, SendPlatformWalletEvent, UpdateBeneficiariesClaimEvent, ZeroSumConfig, BP_DEC};
use crate::{
...
pub struct FinishTournament<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", operator.key().as_ref()], bump, constraint = operator_info.approved && (operator_info.role == Role::BACKEND || operator_info.role == Role::OWNER) @ TournamentError::OperatorNotApprovedOrInvalidRole)]
