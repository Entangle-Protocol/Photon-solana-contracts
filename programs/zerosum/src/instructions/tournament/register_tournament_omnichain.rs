use crate::{
    error::TournamentError, init_tournament_participant_account, ClaimableUserInfo, OperatorInfo, ParticipantsRegistered, Role, Team, Tournament, TournamentStatus, ZS_ROOT
};
...
pub struct RegisterTournamentOmnichain<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", payer.key().as_ref()], bump, constraint = (operator_info.role == Role::OWNER || operator_info.role == Role::MESSENGER) && operator_info.approved @ TournamentError::OperatorNotApprovedOrInvalidRole)]
    ...
    #[account(
        init_if_needed,
        payer = payer,
        seeds = [ZS_ROOT, b"USER", payer.key().as_ref()],
        bump,
...
pub fn handle_register_tournament_omnichain<'info>(
    ...
    for t in new_teammates {
        ...
        let (participant_pda, bump) = Pubkey::find_program_address(&[ZS_ROOT, b"TEAM_PARTICIPANT", &tournament.id.to_le_bytes().as_ref(), t.as_ref()], &ctx.program_id);
        let pda_account = ctx
            .remaining_accounts
            .iter()
