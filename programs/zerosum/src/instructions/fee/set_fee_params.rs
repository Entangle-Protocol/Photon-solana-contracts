...
use crate::{
    ...
    FeeMeta, OperatorInfo, Role, ZeroSumConfig, BP_DEC, ZS_ROOT,
};
...
pub struct SetFeeParams<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", operator.key().as_ref()], bump, constraint = operator_info.approved @ ControlAccessError::OperatorNotApproved, constraint = (operator_info.role == Role::DEVELOPER || operator_info.role == Role::OWNER) @ ControlAccessError::OperatorNotDeveloper)]
    pub operator_info: Box<Account<'info, OperatorInfo>>,

    #[account(init_if_needed, payer = operator, space = FeeMeta::LEN, seeds = [ZS_ROOT, b"FEE_META", &fee_type.to_le_bytes().as_ref()], bump)]
...
pub fn handle_set_fee_params(
    ...
    let config: &mut Box<Account<'_, ZeroSumConfig>> = &mut ctx.accounts.config;
