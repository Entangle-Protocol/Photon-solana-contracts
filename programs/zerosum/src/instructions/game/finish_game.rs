...
pub struct FinishGame<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", operator.key().as_ref()], bump, constraint = operator_info.approved @ ControlAccessError::OperatorNotApproved, constraint = (operator_info.role == Role::MESSENGER || operator_info.role == Role::BACKEND) @ ControlAccessError::OperatorNotApproved)]
...
pub fn handle_finish_game<'info>(
    ...
    for (i, winner) in winners.iter().enumerate() {
        ...
        let (expected_pda, bump) =
            Pubkey::find_program_address(&[ZS_ROOT, b"USER", winner.as_ref()], &ctx.program_id);
