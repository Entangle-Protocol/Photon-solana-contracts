...
use crate::{OperatorInfo, Role, ZeroSumConfig, ZS_ROOT};
...
pub struct Initialize<'info> {
    ...
    #[account(init, payer = admin, space = ZeroSumConfig::LEN, seeds = [ZS_ROOT, b"CONFIG"], bump)]
    pub config: Box<Account<'info, ZeroSumConfig>>,

    #[account(init, payer = admin, space = OperatorInfo::LEN, seeds = [ZS_ROOT, b"OPERATOR", admin.key().as_ref()], bump)]
