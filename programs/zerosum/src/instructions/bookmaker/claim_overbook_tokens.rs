...
pub struct ClaimOverbookTokens<'info> {
    ...
    #[account(mut, seeds = [ZS_ROOT, b"GAMBLER", &tournament_id.to_le_bytes().as_ref(), captain.key().as_ref(), gambler.key().as_ref()], bump)]
    pub gambler_info: Box<Account<'info, GamblerInfo>>,

    #[account(mut, seeds = [ZS_ROOT, b"TOURNAMENT", &tournament_id.to_le_bytes().as_ref()], bump, constraint = tournament.status == TournamentStatus::Started || tournament.status == TournamentStatus::PreCancel || tournament.status == TournamentStatus::Canceled || tournament.status == TournamentStatus::PreFinish || tournament.status == TournamentStatus::Finished @ BookmakerError::InvalidTournamentStatus)]
