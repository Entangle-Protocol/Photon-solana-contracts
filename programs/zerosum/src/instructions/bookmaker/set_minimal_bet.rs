...
pub struct SetMinimalBet<'info> {
    ...
    #[account(seeds = [ZS_ROOT, b"OPERATOR", admin.key().as_ref()], bump, constraint = operator_info.approved && (operator_info.role == Role::DEVELOPER || operator_info.role == Role::OWNER) @ ControlAccessError::OperatorNotApprovedOrInvalidRole)]
