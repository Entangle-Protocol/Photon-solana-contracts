...
pub struct MakeBet<'info> {
    ...
    #[account(mut, seeds = [ZS_ROOT, b"TOURNAMENT", &tournament_id.to_le_bytes().as_ref()], bump)]
    pub tournament: Box<Account<'info, Tournament>>,

    #[account(init_if_needed, payer = payer, space = TournamentBook::LEN, seeds = [ZS_ROOT, b"BOOK", &tournament_id.to_le_bytes().as_ref()], bump)]
    ...
    #[account(init_if_needed, payer = payer, space = CaptainBet::LEN, seeds = [ZS_ROOT, b"CAPTAIN_BET", &tournament_id.to_le_bytes().as_ref(), captain.key().as_ref()], bump)]
    pub captain_bet: Box<Account<'info, CaptainBet>>,

    #[account(init_if_needed, payer = payer, space = GamblerInfo::LEN, seeds = [ZS_ROOT, b"GAMBLER", &tournament_id.to_le_bytes().as_ref(), captain.key().as_ref(), gambler.key().as_ref()], bump)]
