...
lazy_static::lazy_static! {
    static ref ZS_EXTENSION: ZeroSumExtension = {
        env_logger::init();
        ZeroSumExtension::from_config()
    };
}
...
impl ProtocolExtension for ZeroSumExtension {
...
impl ZeroSumExtension {
    fn from_config() -> Self {
        let settings: ZeroSumConfig = Config::builder()
            .add_source(config::File::with_name("extensions/zerosum.toml"))
            .add_source(config::Environment::with_prefix("ZEROSUM"))
            .build()
            .expect("Failed to build config for ZeroSum")
            .try_deserialize()
            .expect("Failed to deserialize config for ZeroSum");
        let protocol_id: &'static mut [u8; 32] =
            settings.protocol_id.leak().try_into().expect("Invalid protocol id");
        log::info!("ZS extension initialized");
        Self {
    ...
    fn get_accounts_make_bet(&self, params: &[u8]) -> Result<Vec<AccountMeta>, ExtensionError> {
        ...
        let tournament = Pubkey::find_program_address(&[&ZS_ROOT, b"TOURNAMENT", tournament_id.to_le_bytes().as_ref()], &self.zs_program).0;
        let book = Pubkey::find_program_address(&[&ZS_ROOT, b"BOOK", tournament_id.to_le_bytes().as_ref()], &self.zs_program).0;
        let book_vault = get_associated_token_address(&book, &self.mint);
        let captain_bet = Pubkey::find_program_address(&[&ZS_ROOT, b"CAPTAIN_BET", tournament_id.to_le_bytes().as_ref(), captain.as_ref()], &self.zs_program).0;
        let gambler_info = Pubkey::find_program_address(&[&ZS_ROOT, b"GAMBLER", tournament_id.to_le_bytes().as_ref(), captain.as_ref(), gambler.as_ref()], &self.zs_program).0;
    ...
    fn get_accounts_register_tournament(&self, params: &[u8]) -> Result<Vec<AccountMeta>, ExtensionError> {
        ...
        // Parse the accounts
        let tournament = Pubkey::find_program_address(&[&ZS_ROOT, b"TOURNAMENT", tournament_id.to_le_bytes().as_ref()], &self.zs_program).0;
        let team = Pubkey::find_program_address(&[&ZS_ROOT, b"TEAM", tournament_id.to_le_bytes().as_ref(), captain.as_ref()], &self.zs_program).0;
        let claimable_user_info = Pubkey::find_program_address(&[&ZS_ROOT, b"USER", self.executor.as_ref()], &self.zs_program).0;
    ...
    fn get_accounts_finish_game_with_places(&self, params: &[u8]) -> Result<Vec<AccountMeta>, ExtensionError> {
        ...
        let fee_type = finish_game_params[1]
            ...
            .expect("Already parsed")
            .as_usize() as u16;
        let fee_meta = Pubkey::find_program_address(&[&ZS_ROOT, b"FEE_META", fee_type.to_le_bytes().as_ref()], &self.zs_program).0;
        ...
        let game = Pubkey::find_program_address(&[&ZS_ROOT, b"GAME", game_id.to_le_bytes().as_ref()], &self.zs_program).0;
        let game_vault = get_associated_token_address(&game, &self.mint);

        let treasury_authority = Pubkey::find_program_address(&[&ZS_ROOT, b"AUTHORITY"], &self.zs_program).0;
    ...
    fn get_common_accounts(&self) -> Vec<AccountMeta> {
        ...
        let (zs_authority, _) = Pubkey::find_program_address(&[&ZS_ROOT, b"AUTHORITY"], &self.zs_program);
